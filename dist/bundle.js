...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__css_index_scss__ = __webpack_require__(7);
 ...
 * 打包的时候生成了一个bundle.js文件，此文件非dist下的bundle.js文件
 * bundle.js文件存在在电脑内存中，也就是在项目根路径下，和dist、src、node_modules平级的！！
 * 
 * 为什么这么做
 * 因为 对UI的更改渲染是比较频繁的，因此如果每次更改都要对dist下的bundle.js文件更改的话，属于对磁盘进行更改，
 * 缓存区域一个是读写快，再一个是避免对磁盘的损耗
 * 
 * 但是，此时的index.html是保存在内存中的吗？而是还保存在物理磁盘上，能不能把index也搞到
 * 内存中呢？
 * 可以的！！
 * 安装插件：html-webpack-plugin
 * 
 * 在引入了.css样式后，webpack发现错误，因为webpack默认只能打包处理.js文件，而其他类型的文件无法处理，
 * 如要处理则需要安装对应的loader加载器（第三方）
 * 
 */
